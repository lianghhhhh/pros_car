...
class DataProcessor:
    ...
    def get_car_control_signal(self):
        signal = self.ros_communicator.get_latest_data("car_control")
        signal = [x.strip() for x in signal.split(":")]
        return signal

    def get_processed_amcl_pose(self):
        amcl_pose_msg = self.ros_communicator.get_latest_data("amcl_pose")
    ...
    def get_camera_x_multi_depth(self):
        if self.ros_communicator.get_latest_data("camera_x_multi_depth") is not None:
            return list(
                self.ros_communicator.get_latest_data("camera_x_multi_depth").data
            )
        else:
            return None

    def get_processed_lidar(self):
        lidar_msg = self.ros_communicator.get_latest_data("lidar")
    ...
    def get_processed_received_global_plan(self):
        received_global_plan_msg = self.ros_communicator.get_latest_data(
    ...
    def get_processed_received_global_plan_no_dynamic(self):
        ...
        goal_position = self.ros_communicator.get_latest_data("goal_pose")
