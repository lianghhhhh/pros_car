...
class ArmControlPublishers:
    """Class to manage common car control publishers and methods"""

    @staticmethod
    def create_publishers(node):
        """Create and return common publishers for arm control"""

        robot_arm_pub = node.create_publisher(
            JointTrajectoryPoint, "robot_arm", 10
        )

        return robot_arm_pub

    @staticmethod
    def create_control_subscription(node, callback):
        """Create subscription for car control signals"""
        return node.create_subscription(String, "arm_control_signal", callback, 10)

    @staticmethod
    def publish_control(node, arm_pub, arm_angle):
        """
